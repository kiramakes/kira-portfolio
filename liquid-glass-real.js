...
// Also falls back to CSS backdrop-filter with shared SVG filter when JS fails.
...
(function () {
    ...
    function calculateRefractionProfile(glassThickness, bezelWidth, heightFn, ior, samples) {
        ...
        for (var i = 0; i < samples; i++) {
            var x = i / samples, y = heightFn(x);
            var dx = x < 1 ? 0.0001 : -0.0001;
            var y2 = heightFn(x + dx), deriv = (y2 - y) / dx, mag = Math.sqrt(deriv * deriv + 1);
    ...
    function ensureSvgDefs() {
        ...
        // Try to find existing defs in the HTML-s embedded SVG
        var existing = document.querySelector('svg.svg-filters defs');
        if (existing) { svgDefs = existing; return svgDefs; }
        // Create our own
    ...
    function processElement(el) {
        try {
            ...
            var old = ensureSvgDefs().querySelector('#' + id);
            if (old) old.remove();
        ...
        } catch (e) { if (window.__lgDebug) console.warn('liquid-glass:', e); }
    }

    var idC = 0;
    var allEls = document.querySelectorAll('.liquid-glass, .lg-btn, .service-item, .project-card, .stat-card, .work-item, .contact-section, .skill-item');
    allEls.forEach(function (el) { if (!el.dataset.lgId) el.dataset.lgId = ++idC; });
    ...
    function processAll() {
        ...
        for (var i = 0; i < els.length; i++) {
            ...
            if (prev > 0 && prev === cur && el.offsetWidth > 0 && processed[key]) continue;
            processed[key] = true;
            processElement(el);
        }
    }
